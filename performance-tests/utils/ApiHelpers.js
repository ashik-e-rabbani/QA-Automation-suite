...
export function addItemToCart(cartId, productId, token) {
    return http.post(`${BASE_URL}/carts/${cartId}/items`, JSON.stringify({productId}), {
        headers: {
            Authorization: token,
            'Content-Type': 'application/json'
        }
...
export function createOrder(cartId, token) {
    if (cartId && token) {
        return http.post(`${BASE_URL}/orders`, JSON.stringify({cartId, customerName: 'Ashik Rabbani'}), {
            headers: {
                Authorization: token,
                'Content-Type': 'application/json'
            }
